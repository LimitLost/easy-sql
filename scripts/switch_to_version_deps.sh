...
# Replaces path dependencies with versions based on internal crate versions.
...
replace_paths_with_versions() {
    echo -e "${YELLOW}Replacing local path dependencies with versions...${NC}"
    ...
    while IFS= read -r -d '' toml_file; do
        ...
        result=$(python3 - "$toml_file" "version" "$DRY_RUN" "${deps_args[@]}" <<'PY'
import re
import sys
...
# Replace path dependencies with versions
replace_paths_with_versions
echo
