...
async fn test_update_functionality() -> anyhow::Result<()> {
    ...
    // Update the row
    ExampleTableIncrement::update(&mut conn, &ExampleInsert { field: 10 }, sql_where!(id = 1))
        .await?;

    // Select and verify
    let updated: ExampleInsert = ExampleTableIncrement::get(&mut conn, sql_where!(id = 1)).await?;
...
async fn test_update_no_match() -> anyhow::Result<()> {
    ...
    ExampleTableIncrement::update(
        &mut conn,
        &ExampleInsert { field: 99 },
        sql_where!(id = 999),
    )
    .await?;
    let row: ExampleInsert = ExampleTableIncrement::get(&mut conn, sql_where!(id = 1)).await?;
...
async fn test_update_multiple_rows() -> anyhow::Result<()> {
    ...
    ExampleTableIncrement::update(&mut conn, &ExampleInsert { field: 42 }, sql_where!(id >= 1))
        .await?;
    let row1: ExampleInsert = ExampleTableIncrement::get(&mut conn, sql_where!(id = 1)).await?;
    let row2: ExampleInsert = ExampleTableIncrement::get(&mut conn, sql_where!(id = 2)).await?;
...
async fn test_update_sql_set_arithmetic() -> anyhow::Result<()> {
    ...
    ExampleTableIncrement::update(&mut conn, sql_set!(field = field + 5), sql_where!(id = 1))
        .await?;
    let row: ExampleInsert = ExampleTableIncrement::get(&mut conn, sql_where!(id = 1)).await?;
...
async fn test_update_rollback() -> anyhow::Result<()> {
    ...
    ExampleTableIncrement::update(&mut conn, &ExampleInsert { field: 99 }, sql_where!(id = 1))
        .await?;
    conn.rollback().await?;
    ...
    let result: Result<ExampleInsert, _> =
        ExampleTableIncrement::get(&mut conn2, sql_where!(id = 1)).await;
